	...
	const bool debug)
{
  ...
  torch::Tensor max_weight_mask = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor first_touched = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
        ...
        max_weight_mask.contiguous().data<int>(),
        first_touched.contiguous().data<int>(),
        debug);
  }
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, out_depth, out_opaticy, n_touched, max_weight_mask, first_touched);
}
